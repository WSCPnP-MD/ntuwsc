...
function wpbf_child_scripts() {
	...
	if ( is_page( 15 ) ) {
		...
		wp_enqueue_style( 'owl-carousel-s', WPBF_CHILD_THEME_URI . '/assets/owlCarousel/owl.carousel.min.css', array(), time());
		...
		wp_enqueue_script( 'owl-carousel-script', WPBF_CHILD_THEME_URI . '/assets/owlCarousel/owl.carousel.min.js', array('jquery'), time());
	}
	else {
	// remove the styles and scripts after leaving the about us page
